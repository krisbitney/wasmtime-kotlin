...
class Caller(val caller: CPointer<wasmtime_caller_t>) {
     ...
     */
    val context: io.github.krisbitney.wasmtime.Context<Any?>
        get() {
            val ptr = wasmtime_caller_context(caller) ?: throw Exception("failed to get caller context")
            return io.github.krisbitney.wasmtime.Context(ptr)
        }
     ...
     */
    fun exportGet(name: String): io.github.krisbitney.wasmtime.Extern? = memScoped {
        ...
        return if (found) {
            io.github.krisbitney.wasmtime.Extern.Companion.fromCValue(context.context, wasmExtern.ptr)
        } else {
            null
        }
    }
